...
import { ShaderProgram } from 'src/assets/js/ShaderProgram.js';
...
export class SnowflakeComponent {
  isStrongSnow: boolean = false;
  flakeLayers: any = null;

  constructor(private LoadAssetsService: LoadAssetsService) { }
  ...
  initializeSnow() {
    if (this.flakeLayers && this.flakeLayers.length == 2)
      this.toggleFlakeLayers();
    else this.buildFlakeLayer();
  }
  ...
  buildFlakeLayer() {
    ...
    const count = this.isStrongSnow ? 5000 : 2000;
    let wind = {
      current: 0,
      force: 0.1,
      target: 0.1,
    ...
    // TODO: Get back here to handle snow speed when Tabrez decide it.
    const snowSpeed = 0.1;

    const snowBGEle = document.getElementById("snowBG");
    if (!snowBGEle) return;
    new ShaderProgram(snowBGEle, {
      depthTest: false,
      texture: snowflake,
      uniforms: {
    ...
    this.flakeLayers = document.querySelectorAll("#snowBG > canvas");
  }

  toggleFlakeLayers() {
    this.flakeLayers[0].classList.toggle("active");
    this.flakeLayers[1].classList.toggle("active");
  }
